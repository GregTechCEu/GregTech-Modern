...
public class AEItemDisplayWidget extends Widget {
    ...
    public void drawInForeground(@NotNull PoseStack graphics, int mouseX, int mouseY, float partialTicks) {
        if (isMouseOverElement(mouseX, mouseY)) {
            GenericStack item = this.gridWidget.getAt(this.index);
            if (item != null) {
                DrawerHelper.drawTooltip(graphics, mouseX, mouseY, List.of(), item.what() instanceof AEItemKey key ? new ItemStack(key.getItem(), (int) item.amount()) : ItemStack.EMPTY, null, Minecraft.getInstance().font);
            }
        }
    }
