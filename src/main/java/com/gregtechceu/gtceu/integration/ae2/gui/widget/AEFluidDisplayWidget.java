    ...
    public void drawInForeground(@NotNull PoseStack graphics, int mouseX, int mouseY, float partialTicks) {
        if (isMouseOverElement(mouseX, mouseY)) {
            ...
            if (fluid != null) {
                ...
                TooltipsHandler.appendFluidTooltips(fluidStack.getFluid(), tooltips, TooltipFlag.Default.NORMAL);
                DrawerHelper.drawTooltip(graphics, mouseX, mouseY, tooltips, ItemStack.EMPTY, null, Minecraft.getInstance().font);
            }
        }
    }
