...
public class MachineModeProvider implements IProbeInfoProvider {
    ...
    public void addProbeInfo(ProbeMode probeMode, IProbeInfo iProbeInfo, Player player, Level level,
        ...
        if (level.getBlockEntity(iProbeHitData.getPos()) instanceof IRecipeLogicMachine recipeLogicMachine) {
            GTRecipeType[] recipeTypes = recipeLogicMachine.self().getDefinition().getRecipeTypes();
