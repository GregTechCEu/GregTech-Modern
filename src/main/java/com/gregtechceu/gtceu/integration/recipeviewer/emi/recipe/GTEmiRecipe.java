    ...
    public List<EmiIngredient> getInputs() {
        ...
        for (var itemContent: items) {
            float chance = (float) recipe.recipeType.getChanceFunction()
                    .getBoostedChance(itemContent, RecipeHelper.getRecipeEUtTier(recipe), RecipeHelper.getRecipeEUtTier(recipe)) / itemContent.maxChance();

            var mapped = ItemRecipeCapability.mapIngredientToEntryList(ItemRecipeCapability.CAP.of(itemContent.content()));
        ...
        for (var fluidContent: fluids) {
            float chance = (float) recipe.recipeType.getChanceFunction()
                    .getBoostedChance(fluidContent, RecipeHelper.getRecipeEUtTier(recipe), RecipeHelper.getRecipeEUtTier(recipe)) / fluidContent.maxChance();

            var mapped = FluidRecipeCapability.mapIngredientToEntryList(FluidRecipeCapability.CAP.of(fluidContent.content()));
