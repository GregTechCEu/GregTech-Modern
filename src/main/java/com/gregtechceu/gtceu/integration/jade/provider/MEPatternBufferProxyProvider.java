    ...
    public void appendTooltip(ITooltip iTooltip, BlockAccessor blockAccessor, IPluginConfig iPluginConfig) {
        if (blockAccessor.getBlockEntity() instanceof MEPatternBufferProxyPartMachine) {
            ...
            if (!serverData.getBoolean("bound")) {
                iTooltip.add(Component.translatable("integration.gtceu.jade.pattern_buffer.not_bound").withStyle(ChatFormatting.RED));
                return;
            }

            int[] pos = serverData.getIntArray("pos");
            iTooltip.add(Component.translatable("integration.gtceu.jade.pattern_buffer.bound_to_pos", pos[0], pos[1], pos[2])
