...
public class GTBedrockOreVeinKubeEvent implements KubeEvent {
    ...
    public void modify(Context cx, ResourceLocation id, Consumer<BedrockOreBuilder> consumer) {
        BedrockOreDefinition vein = this.registry.get(id);
        if (vein == null) {
            throw new KubeRuntimeException("Bedrock ore vein doesn't exist: " + id)
                    .source(SourceLine.of(cx));
        }

        BedrockOreBuilder builder = BedrockOreBuilder.from(vein, id);
        consumer.accept(builder);
        register(id, builder.createTransformedObject());
    }
    ...
    public void remove(Context cx, ResourceLocation id) {
        ...
        var holder = this.registry.getHolderOrThrow(createKey(id));
    ...
    private static ResourceKey<BedrockOreDefinition> createKey(ResourceLocation id) {
