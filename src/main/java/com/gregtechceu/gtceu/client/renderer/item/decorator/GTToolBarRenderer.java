...
public class GTToolBarRenderer implements IItemDecorator {
    ...
    @Override
    public boolean render(@NotNull GuiGraphicsExtractor GuiGraphicsExtractor, @NotNull Font font, ItemStack stack,
                          int x, int y) {
        if (stack.getItem() instanceof IGTTool gtTool) {
            return ToolChargeBarRenderer.renderBarsTool(GuiGraphicsExtractor, gtTool, stack, x, y);
        }
        return false;
    }
}
