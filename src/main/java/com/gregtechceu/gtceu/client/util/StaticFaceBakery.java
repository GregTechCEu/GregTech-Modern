...
public class StaticFaceBakery {
    ...
    private static void fillVertex(int[] vertexData, int vertexIndex, Vector3f vector, TextureAtlasSprite sprite, BlockFaceUV blockFaceUV) {
        ...
        vertexData[i + UV_INDEX] = Float.floatToRawIntBits(
            sprite.getU((double)blockFaceUV.getU(vertexIndex) * 0.999 + (double)blockFaceUV.getU((vertexIndex + 2) % 4) * 0.001)
        );
        vertexData[i + 4 + 1] = Float.floatToRawIntBits(
            sprite.getV((double)blockFaceUV.getV(vertexIndex) * 0.999 + (double)blockFaceUV.getV((vertexIndex + 2) % 4) * 0.001)
        );
    }
