...
public final class GTQuadTransformers {
    ...
    public static BakedQuad setSprite(BakedQuad quad, TextureAtlasSprite sprite) {
        ...
        for (int i = 0; i < 4; i++) {
            int offset = i * IQuadTransformer.STRIDE + IQuadTransformer.UV0;
            float originalU = Float.intBitsToFloat(vertices[offset]);
            float originalV = Float.intBitsToFloat(vertices[offset + 1]);
            float uOffset = originalSprite.getUOffset(originalU);
            float vOffset = originalSprite.getVOffset(originalV);

            vertices[offset] = Float.floatToRawIntBits(sprite.getU(uOffset));
            vertices[offset + 1] = Float.floatToRawIntBits(sprite.getV(vOffset));
        }
