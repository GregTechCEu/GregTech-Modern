...
public record QuadInfo(TextureAtlasSprite sprite, int tintIndex, Direction direction,
    ...
    public Vector2f[] normalizeUVs() {
        ...
        for (int i = 0; i < ret.length; i++) {
            ret[i] = ModelUtils.normalizeUV(sprite, uvs[i]);
        }
        return ret;
    }
    ...
    public Vector2f[] relativizeUVs() {
        ...
        for (int i = 0; i < uvs.length; i++) {
            ret[i] = ModelUtils.relativizeUV(sprite, uvs[i]);
        }
        return ret;
    }

    public int getNormalizedUVQuadrant() {
        return getUVQuadrant(ModelUtils.normalizeUV(sprite, maxUV));
    }
    ...
    public Vector2f[] transformUVData(TextureAtlasSprite other, ISubmap submap) {
        ...
        Vector2f[] newUVs = {
                new Vector2f(uvs[0].x() == this.minUV.x() ? minU : maxU, uvs[0].y() == this.minUV.y() ? minV : maxV),
                new Vector2f(uvs[1].x() == this.minUV.x() ? minU : maxU, uvs[1].y() == this.minUV.y() ? minV : maxV),
                new Vector2f(uvs[2].x() == this.minUV.x() ? minU : maxU, uvs[2].y() == this.minUV.y() ? minV : maxV),
                new Vector2f(uvs[3].x() == this.minUV.x() ? minU : maxU, uvs[3].y() == this.minUV.y() ? minV : maxV)
        };
        return ModelUtils.relativizeUVs(other, newUVs);
    }
    ...
    private Pair<QuadInfo, QuadInfo> divide(boolean vertical) {
        ...
        if (minUV < 0.5f && maxUV > 0.5f) {
            float delta = GTMath.normalize(minUV, maxUV, 0.5f);

            Vector2f[] firstUVs = ModelUtils.relativizeUVs(sprite,
            ...
            Vector2f[] secondUVs = ModelUtils.relativizeUVs(sprite,
