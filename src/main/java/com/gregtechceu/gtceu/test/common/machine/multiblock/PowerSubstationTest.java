 ...
 * helper.assertTrue(storage.getCapacity().equals(BigInteger.valueOf(100)), "Wrong max storage! was" +
 * storage.getCapacity() + ", expected 100");
 * 
 * // Random fill and drain tests
 * long filled = storage.fill(50);
 * helper.assertTrue(filled == 50, "Expected `fill` to return 50, was" + filled);
 * helper.assertTrue(storage.getStored().equals(BigInteger.valueOf(50)), "Expected stored energy amount to be 50, was" +
 * storage.getStored());
 * filled = storage.fill(100);
 * helper.assertTrue(filled == 50, "Expected `fill` to return 50, was" + filled);
 * helper.assertTrue(storage.getStored().equals(BigInteger.valueOf(100)), "Expected stored energy amount to be 100, was"
 * + storage.getStored());
 * filled = storage.fill(100);
 ...
 * }
 * 
 * private static PowerSubstationMachine.PowerStationEnergyBank createStorage(MetaMachine machine, long...
 * storageValues) {
 ...
 * }
 * 
 * private static PowerSubstationMachine.PowerStationEnergyBank
 * rebuildStorage(PowerSubstationMachine.PowerStationEnergyBank storage, long... storageValues) {
 * List<IBatteryData> batteries = new ArrayList<>();
 * for (long value : storageValues) {
 * batteries.add(new TestBattery(value));
 * }
