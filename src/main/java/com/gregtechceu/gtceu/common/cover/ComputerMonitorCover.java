...
public class ComputerMonitorCover extends CoverBehavior
    ...
    public Widget createUIWidget() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int finalI = i;
            if (i >= getFormatStringLines().size()) getFormatStringLines().add("");
            formatStringInput.setCurrentString(getFormatStringLines().get(i));
            formatStringInput.setTextResponder((s) -> getFormatStringLines().set(finalI, s));
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int finalI = i;
            if (i >= getFormatStringArgs().size()) getFormatStringArgs().add("");
            formatStringArgsInput.setCurrentString(getFormatStringArgs().get(i));
            formatStringArgsInput.setTextResponder((s) -> getFormatStringArgs().set(finalI, s));
