...
public class GTResearchMachines {
    ...
    public static final MultiblockMachineDefinition RESEARCH_STATION = REGISTRATE
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .where('P', blocks(COMPUTER_CASING.get())
                            .and(abilities(PartAbility.INPUT_ENERGY).withMinGlobalLimited(1).withMaxGlobalLimited(2, 1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_RECEPTION).withExactLimit(1))
    ...
    public static final MultiblockMachineDefinition DATA_BANK = REGISTRATE.multiblock("data_bank", DataBankMachine::new)
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .where('D', blocks(COMPUTER_CASING.get()).withMinGlobalLimited(3)
                            .and(abilities(PartAbility.DATA_ACCESS).withPreviewCount(3))
                            .and(abilities(PartAbility.OPTICAL_DATA_TRANSMISSION).withMinGlobalLimited(1, 1))
                            .and(abilities(PartAbility.OPTICAL_DATA_RECEPTION).withPreviewCount(1)))
                    .where('A', blocks(COMPUTER_CASING.get()))
                    .where('C', blocks(HIGH_POWER_CASING.get()).withMinGlobalLimited(4)
                            .and(abilities(PartAbility.INPUT_ENERGY).withMinGlobalLimited(1).withMaxGlobalLimited(2, 1))
                            .and(autoAbilities(true, false, false)))
                    .build())
    ...
    public static final MultiblockMachineDefinition NETWORK_SWITCH = REGISTRATE
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .where('S', controller(blocks(definition.getBlock())))
                    .where('A', blocks(ADVANCED_COMPUTER_CASING.get()))
                    .where('X', blocks(COMPUTER_CASING.get()).withMinGlobalLimited(7)
                            .and(abilities(PartAbility.INPUT_ENERGY).withMinGlobalLimited(1).withMaxGlobalLimited(2, 1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_TRANSMISSION).withMinGlobalLimited(1, 1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_RECEPTION).withMinGlobalLimited(1, 2))
                            .and(autoAbilities(true, false, false)))
                    .build())
    ...
    public static final MultiblockMachineDefinition HIGH_PERFORMANCE_COMPUTING_ARRAY = REGISTRATE
            ...
            .pattern(definition -> MultiblockPatternBuilder.start(FRONT, UP, RIGHT)
                    ...
                    .where('C', blocks(COMPUTER_CASING.get()).withMinGlobalLimited(5)
                            .and(abilities(PartAbility.INPUT_ENERGY).withMinGlobalLimited(1).withMaxGlobalLimited(2, 1))
                            .and(abilities(PartAbility.IMPORT_FLUIDS).withMaxGlobalLimited(1))
                            .and(abilities(PartAbility.COMPUTATION_DATA_TRANSMISSION).withExactLimit(1))
                            .and(autoAbilities(true, false, false)))
                    .build())
            .sidedWorkableCasingModel(GTCEu.id("block/casings/hpca/computer_casing"),
