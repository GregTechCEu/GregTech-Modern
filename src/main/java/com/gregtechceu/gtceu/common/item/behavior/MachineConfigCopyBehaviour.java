...
public class MachineConfigCopyBehaviour implements IInteractionItem, IAddInformation {
    ...
    private static final Component ENABLED = Component.translatable("item.gtceu.behavior.memory_card.enabled");
    private static final Component DISABLED = Component.translatable("item.gtceu.behavior.memory_card.disabled");
    ...
    private static void addConfigTooltips(List<Component> tooltip, CompoundTag tag) {
        tooltip.add(Component.translatable("item.gtceu.behavior.memory_card.copy_target", tag.getString(COPY_SOURCE)));
        ...
        if (tag.contains(ITEM_OUTPUT_SIDE) && tag.contains(ITEM_AUTO_OUTPUT) && tag.contains(ALLOW_ITEM_IN_FROM_OUT)) {
            ...
            if (tag.getBoolean(ITEM_AUTO_OUTPUT) && tag.getBoolean(ALLOW_ITEM_IN_FROM_OUT))
                ...
                outputMode = Component.translatable("item.gtceu.behavior.memory_card.setting.tooltip.allow_input");
            else outputMode = Component.empty();

            Direction dir = stringToDirection(tag.getString(ITEM_OUTPUT_SIDE));
            if (dir == null) return;

            tooltip.add(Component.translatable("item.gtceu.behavior.memory_card.setting.tooltip.item_io",
        ...
        if (tag.contains(FLUID_OUTPUT_SIDE) && tag.contains(FLUID_AUTO_OUTPUT) &&
            ...
            tooltip.add(Component.translatable("item.gtceu.behavior.memory_card.setting.tooltip.fluid_io",
                    Component.literal(DIRECTION_STRINGS[dir.ordinal()]), outputMode));
        }
        ...
        if (tag.contains(ITEMS_TO_PASTE)) {
            ...
            tooltip.add(Component.empty());
            tooltip.add(Component.translatable("item.gtceu.behavior.memory_card.tooltip.items_to_paste"));
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltipComponents,
        ...
        tooltipComponents.add(Component.translatable("item.gtceu.behavior.memory_card.tooltip.copy"));
        tooltipComponents.add(Component.translatable("item.gtceu.behavior.memory_card.tooltip.paste"));
        ...
        if (Screen.hasShiftDown()) {
            ...
            tooltipComponents.add(Component.translatable("item.gtceu.behavior.memory_card.tooltip.view_stored"));
        }
    }
}
