...
public class FisherMachine extends TieredEnergyMachine
    ...
    public void fishingUpdate() {
        ...
        if (progress >= maxProgress) {
            var lootTableRegistry = getLevel().registryAccess().lookupOrThrow(Registries.LOOT_TABLE);
            LootTable lootTable = lootTableRegistry.getValueOrThrow(BuiltInLootTables.FISHING);
            if (!this.junkEnabled) {
                lootTable = lootTableRegistry.getValueOrThrow(BuiltInLootTables.FISHING_FISH);
            }
    ...
    public static BiFunction<Identifier, Integer, EditableMachineUI> EDITABLE_UI_CREATOR = Util
            .memoize(FisherMachine::createEditableUI);

    public static EditableMachineUI createEditableUI(Identifier path, int inventorySize) {
