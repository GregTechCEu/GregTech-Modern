    ...
    public Widget createUIWidget() {
        ...
        for (int y = 0; y < tanks; y++) {
            for (int x = 0; x < tanks; x++) {
                container.addWidget(new SlotWidget(
                        getInventory().storage, index++, 4 + x * 18, 4 + y * 18, true,
                        getIo().support(IO.IN))
                        .setBackgroundTexture(GuiTextures.SLOT)
                        .setIngredientIO(getIo() == IO.IN ? IngredientIO.INPUT :
                                getIo() == IO.OUT ? IngredientIO.OUTPUT : IngredientIO.BOTH));
            }
        }
        ...
        for (int y = 0; y < tanks; y++) {
            container.addWidget(new TankWidget(
                    tank.getStorages()[index++], 4 + tanks * 18, 4 + y * 18, true,
                    getIo().support(IO.IN))
