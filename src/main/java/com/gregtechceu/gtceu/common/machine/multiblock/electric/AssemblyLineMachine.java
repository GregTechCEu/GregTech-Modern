...
public class AssemblyLineMachine extends WorkableElectricMultiblockMachine {
    ...
    private boolean checkFluidInputs(GTRecipe recipe, boolean isTick) {
        ...
        for (int i = 0; i < inputsSize; i++) {
            ...
            if (!recipeStack.test(fluidStack) || (recipeStack instanceof IRangedIngredient ? ((IRangedIngredient)recipeStack).getMaxRoll() : recipeStack.getAmount()) > fluidStack.getAmount()) {
                return false;
            }
        }
        return true;
