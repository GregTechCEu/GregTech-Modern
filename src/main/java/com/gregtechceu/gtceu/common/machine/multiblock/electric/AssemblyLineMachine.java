    ...
    public boolean beforeWorking(@Nullable GTRecipe recipe) {
        if (ConfigHolder.INSTANCE.machines.orderedAssemblyLineItems) {
            ...
            for (int i = 0; i < recipeInputs.size(); i++) {
                ...
                SizedIngredient recipeStack = ItemRecipeCapability.CAP.of(recipeInputs.get(i).content);
            ...
            if (ConfigHolder.INSTANCE.machines.orderedAssemblyLineFluids) {
                ...
                for (int i = 0; i < recipeInputs.size(); i++) {
                    ...
                    SizedFluidIngredient recipeStack = FluidRecipeCapability.CAP.of(recipeInputs.get(i).content);
                    if (!recipeStack.test(fluidStack) || recipeStack.amount() > fluidStack.getAmount()) {
                        return false;
                    }
                }
            }
