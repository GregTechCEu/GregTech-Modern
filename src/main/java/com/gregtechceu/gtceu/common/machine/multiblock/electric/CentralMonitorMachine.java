...
public class CentralMonitorMachine extends WorkableElectricMultiblockMachine
    ...
    final Set<IMonitorComponent> selectedComponents = new HashSet<>();
    final List<IMonitorComponent> selectedTargets = new ArrayList<>();
    ...
    boolean isInAnyGroup(IMonitorComponent component) {
    ...
    public Widget createUIWidget() {
        Widget builderWidget = super.createUIWidget();
            ...
            Runnable changeListener = () -> {
                ...
                if (slot.getItem().getItem() instanceof IComponentItem item) {
                    for (IItemComponent component : item.getComponents()) {
                        if (component instanceof IMonitorModuleItem module &&
                                module.createUIWidget(slot.getItem(), this, group) instanceof Widget widget) {
        ...
        createGroupButton.setOnPressCallback(click -> {
            MonitorGroup group = new MonitorGroup(
                    Component.translatable("gtceu.gui.central_monitor.group_default_name",
                            monitorGroups.size() + 1).getString());
        ...
        int startX = 20;
        int startY = 30;
        for (int row = 0; row <= getDownDist() + getUpDist(); row++) {
            imageButtons.add(new ArrayList<>());
            for (int col = 0; col <= getLeftDist() + getRightDist(); col++) {
                ...
                Consumer<Iterator<IMonitorComponent>> callback = (it) -> {
                    ...
                    if (selectedComponents.contains(component)) {
                        ...
                        createGroupButton.setVisible(
                                selectedComponents.stream().noneMatch(this::isInAnyGroup));
                        removeFromGroupButton.setVisible(
                                selectedComponents.stream().allMatch(this::isInAnyGroup));
                        ...
                        ColorRectTexture rect = new ColorRectTexture(
                                (selectedTargets.isEmpty() || selectedTargets.get(0) != component) ?
    ...
    private IGuiTexture getComponentTexture(int row, int col) {
        if (row < 0 || col < 0 ||
                row > getDownDist() + getUpDist() + 1 ||
                col > getLeftDist() + getRightDist() + 1) {
