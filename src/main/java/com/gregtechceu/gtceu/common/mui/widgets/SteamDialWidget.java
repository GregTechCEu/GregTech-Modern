...
public class SteamDialWidget implements IDrawable {
    ...
    public void draw(GuiContext context, int x, int y, int width, int height, WidgetTheme widgetTheme) {
        ...
        bufferBuilder.vertex(pose, x + height * sinAngle, y - height * cosAngle, 0.0f).color(r, g, b, a).endVertex();
        bufferBuilder.vertex(pose, x - height * sinAngle, y + height * cosAngle, 0.0f).color(r, g, b, a).endVertex();
        bufferBuilder.vertex(pose, x - height * cosAngle, y - height * sinAngle, 0.0f).color(r, g, b, a).endVertex();
