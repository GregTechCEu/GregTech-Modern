...
public class QuadLighterMixin {
    ...
    private void gtceu$renderToEmissiveBuffer(VertexConsumer instance, PoseStack.Pose poseEntry, BakedQuad quad,
        ...
        BlockPos chunkPos = BloomEffectUtil.CURRENT_RENDERING_CHUNK_POS.get();
        // Check if quad is full brightness OR we have bloom enabled for the quad
        if (GTShaders.allowedShader() && chunkPos != null && (!quad.isShade() || GTMetadataSection.hasBloom(quad.getSprite()))) {
            original.call(BloomEffectUtil.getOrStartBloomBuffer(chunkPos), poseEntry, quad,
