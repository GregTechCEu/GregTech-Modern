...
public class SodiumFaceLayerMixin {
    ...
    private MutableQuadViewImpl gtceu$preserveFaceLayer(MutableQuadViewImpl quadView, @Local BakedQuad quad) {
        int tag = FaceLayerRouting.encodeTag(quad);
        if (tag != 0) {
            quadView.tag(tag);
        }
        return quadView;
