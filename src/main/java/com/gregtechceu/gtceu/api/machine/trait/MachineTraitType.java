 ...
 * @param parentTraitType         The parent trait type (or null if this trait is a direct subclass of
 *                                {@link MachineTrait}
...
public record MachineTraitType<T extends MachineTrait>(Class<T> traitClass,
    ...
    public MachineTraitType(Class<T> traitClass, @Nullable MachineTraitType<? super T> parentTraitType,
        ...
        if (parentTraitType == null) {
            if (traitClass.getSuperclass() != MachineTrait.class) throw new IllegalStateException(
                    ("Error creating machine trait type for %s: Expected a parent trait type")
                            .formatted(traitClass));
        } else {
            if (traitClass.getSuperclass() == MachineTrait.class) {
                throw new IllegalStateException(
                        ("Error creating machine trait type for %s: Trait superclass is MachineTrait, but a parent trait type was provided")
                                .formatted(traitClass));
            } else if (!traitClass.getSuperclass().equals(parentTraitType.traitClass)) {
                throw new IllegalStateException(
                        ("Error creating machine trait type for %s: Trait superclass is %s, but parent trait type is for %s")
        ...
        if (parentTraitType != null && !parentTraitType.allowsMultipleInstances && allowsMultipleInstances) {
            throw new IllegalStateException(
                    ("Error creating machine trait type for %s: allowMultipleInstances is true, but the parent trait type does not allow multiple instances")
                            .formatted(traitClass));
        }
    }
