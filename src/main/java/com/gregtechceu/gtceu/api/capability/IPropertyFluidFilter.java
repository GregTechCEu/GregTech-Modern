    ...
    default void appendTooltips(@NotNull List<Component> tooltip, boolean showToolsInfo, boolean showTemperatureInfo) {
        if (GTUtil.isShiftDown()) {
            if (showTemperatureInfo)
                tooltip.add(Component.translatable("gtceu.fluid_container.max_temperature",
                        FormattingUtil.formatNumbers(getMaxFluidTemperature())));
            if (isGasProof()) tooltip.add(Component.translatable("gtceu.fluid_container.gas_proof"));
            else tooltip.add(Component.translatable("gtceu.fluid_container.not_gas_proof"));
            if (isPlasmaProof()) tooltip.add(Component.translatable("gtceu.fluid_container.plasma_proof"));
            if (isCryoProof()) tooltip.add(Component.translatable("gtceu.fluid_container.cryo_proof"));
