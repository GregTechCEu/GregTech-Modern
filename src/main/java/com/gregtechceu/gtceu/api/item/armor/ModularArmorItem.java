...
public class ModularArmorItem extends ArmorItem implements IComponentItem {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        for (IItemComponent component : components) {
            if (component instanceof IAddInformation addInformation) {
                addInformation.appendHoverText(stack, context, tooltipComponents, tooltipFlag);
            }
        }
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand usedHand) {
        for (IItemComponent component : components) {
            ...
            if (component instanceof IInteractionItem interactionItem) {
                var result = interactionItem.use(itemstack, level, player, usedHand);
    ...
    public void attachCapabilities(RegisterCapabilitiesEvent event) {
        for (IItemComponent component : components) {
            if (component instanceof IComponentCapability componentCapability) {
                componentCapability.attachCapabilities(event, this);
            }
        }
    }
}
