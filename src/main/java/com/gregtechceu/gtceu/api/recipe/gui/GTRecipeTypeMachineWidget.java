        ...
        if (progressWidget.hasChildren()) {
            for (IWidget w : progressWidget.getChildren()) {
                if (w instanceof ITooltip<?> tooltipWidget) {
                    tooltipWidget
                            .tooltip(r -> r.add(Text.comp(Component.translatable("gui.gtceu.recipe.show_recipes"))));
                }
            }
        } else {
            progressWidget.tooltip(r -> r.add(Text.comp(Component.translatable("gui.gtceu.recipe.show_recipes"))));
        }
