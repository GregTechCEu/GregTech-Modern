...
public abstract class RecipeCondition<T extends RecipeCondition<T>> {
    ...
    @NotNull
    public final JsonObject serialize(HolderLookup.Provider provider) {
        var ops = RegistryOps.create(JsonOps.INSTANCE, provider);
        return CODEC.encodeStart(ops, this).getOrThrow().getAsJsonObject();
    }

    public static RecipeCondition<?> deserialize(HolderLookup.Provider provider, @NotNull JsonObject config) {
        var ops = RegistryOps.create(JsonOps.INSTANCE, provider);
