...
public class ObjectArrayTransformer<T> implements ValueTransformer<T[]> {
    ...
    public @Nullable T @Nullable [] deserializeNBT(Tag tag, ValueTransformer.TransformerContext<T[]> context) {
        ...
        for (int i = 0; i < listTag.size(); i++) {
            T result = elementTransformer.deserializeNBT(listTag.get(i),
    ...
    @Override
    public void writeToPacket(RegistryFriendlyByteBuf buf, @Nullable T[] value, TransformerContext<T[]> context) {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public @Nullable T @Nullable [] readFromPacket(RegistryFriendlyByteBuf buf, TransformerContext<T[]> context) {
        @Nullable
