...
public class MonitorCoverTest {
    ...
    public static void testEnergyPlaceholders(GameTestHelper helper) {
        BatteryBufferMachine machine = (BatteryBufferMachine) TestUtils.setMachine(helper, new BlockPos(0, 1, 0), GTMachines.BATTERY_BUFFER_4[GTValues.HV]);
        machine.getBatteryInventory().insertItem(0, GTItems.BATTERY_HV_LITHIUM.asStack(), false);
        ComputerMonitorCover cover = (ComputerMonitorCover) TestUtils.placeCover(helper, machine, GTItems.COVER_SCREEN.asStack(), Direction.UP);
        ...
        helper.runAtTickTime(1, () -> {
            TestUtils.assertEqual(helper, cover.getText(), "Energy: 0/" + machine.energyContainer.getEnergyCapacity() + " EU");
            helper.succeed();
        });
    }
    ...
    public static void testCombinePlaceholder(GameTestHelper helper) {
        BatteryBufferMachine machine = (BatteryBufferMachine) TestUtils.setMachine(helper, new BlockPos(0, 1, 0), GTMachines.BATTERY_BUFFER_4[GTValues.HV]);
        ComputerMonitorCover cover = (ComputerMonitorCover) TestUtils.placeCover(helper, machine, GTItems.COVER_SCREEN.asStack(), Direction.UP);
        cover.getFormatStringLines().add("{if 1 {combine test 1 2 3 lol {repeat 5 \"a \"}} \"if placeholder failed somehow\"}");
        cover.setUpdateInterval(1);
        helper.runAtTickTime(1, () -> {
