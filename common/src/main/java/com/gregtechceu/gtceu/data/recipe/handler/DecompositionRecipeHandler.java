...
public class DecompositionRecipeHandler {
    ...
    private static void processDecomposition(TagPrefix decomposePrefix, Material material, Consumer<FinishedRecipe> provider) {
        ...
        if (decomposePrefix != null) {
            ...
            var smallestMaterialAmount = getSmallestMaterialAmount(materialAmounts);
        ...
        if (material.hasFlag(DECOMPOSITION_BY_ELECTROLYZING)) {
            builder = ELECTROLYZER_RECIPES.recipeBuilder("decomposition_electrolyzing_", material.getName())
                    .duration((int) (material.getProtons() * totalInputAmount * 2))
                    .EUt(material.getMaterialComponents().size() <= 2 ? VA[LV] : 2L * VA[LV]);
        } else {
            builder = CENTRIFUGE_RECIPES.recipeBuilder("decomposition_electrolyzing_", material.getName())
    ...
    private static boolean isEveryMaterialReducible(int divisor, List<Integer> materialAmounts) {
        for (var amount : materialAmounts) {
            if (amount % divisor != 0)
                return false;
        }
        return true;
