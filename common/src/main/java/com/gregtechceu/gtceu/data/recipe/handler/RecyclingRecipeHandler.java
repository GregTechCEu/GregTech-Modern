...
public class RecyclingRecipeHandler {
    ...
    private static final List<Object> CRUSHING_PREFIXES = Arrays.asList(
            ingot, gem, stick, plate, plank,
            ring, stickLong, foil, bolt,
            screw, nugget, gearSmall, gear,
            frameGt, plateDense, spring,
            springSmall, block, wireFine,
            rotor, lens, turbineBlade,
            round, plateDouble, dust,
            (Predicate<TagPrefix>) tagPrefix -> tagPrefix.name().startsWith("toolHead"),
            (Predicate<TagPrefix>) tagPrefix -> tagPrefix.name().startsWith("gem"),
            (Predicate<TagPrefix>) tagPrefix -> tagPrefix.name().startsWith("cableGt"),
            (Predicate<TagPrefix>) tagPrefix -> tagPrefix.name().startsWith("wireGt"),
            (Predicate<TagPrefix>) tagPrefix -> tagPrefix.name().startsWith("pipe")
    );

    private static final List<TagPrefix> IGNORE_ARC_SMELTING = Arrays.asList(
            ingot, gem, nugget);

    public static void init(Consumer<FinishedRecipe> provider) {
        //registers universal maceration recipes for specified ore prefixes
        for (var tagPrefix : values()) {
            if (CRUSHING_PREFIXES.stream().anyMatch(object -> {
                if (object instanceof TagPrefix)
                    return object == tagPrefix;
                else if (object instanceof Predicate)
                    return ((Predicate<TagPrefix>) object).test(tagPrefix);
                else return false;
            })) tagPrefix.executeHandler(PropertyKey.DUST, (orePrefix, material, property) -> processCrushing(orePrefix, material, property, provider));
        }
    }
    ...
    public static void processCrushing(TagPrefix thingPrefix, Material material, DustProperty property, Consumer<FinishedRecipe> provider) {
        ...
        registerRecyclingRecipes(new UnificationEntry(thingPrefix, material), materialStacks, ignoreArcSmelting, thingPrefix, provider);
    }

    public static void registerRecyclingRecipes(UnificationEntry input, List<MaterialStack> components, boolean ignoreArcSmelting, @Nullable TagPrefix prefix, Consumer<FinishedRecipe> provider) {
        ...
        if (prefix != dust) {
            registerMaceratorRecycling(input, components, voltageMultiplier, provider);
        }
        if (prefix != null) {
            registerExtractorRecycling(input, components, voltageMultiplier, prefix, provider);
        }
        ...
        if (components.size() == 1) {
            ...
            if (input == null || input.tagPrefix == ingot && m.getProperty(PropertyKey.INGOT).getArcSmeltInto() == m) {
                return;
            }

            // Prevent Magnetic dust -> Regular Ingot Arc Furnacing, avoiding the EBF recipe
            // "I will rework magnetic materials soon" - DStrand1
            if(prefix == dust && m.hasFlag(IS_MAGNETIC)) {
                return;
            }
        }
        registerArcRecycling(input, components, prefix, provider);
    }

    private static void registerMaceratorRecycling(UnificationEntry input, List<MaterialStack> materials, int multiplier, Consumer<FinishedRecipe> provider) {

        // Finalize the output list.
        List<ItemStack> outputs = finalizeOutputs(
                materials,
                MACERATOR_RECIPES.getMaxOutputs(ItemRecipeCapability.CAP),
        ...
        MACERATOR_RECIPES.recipeBuilder(input, "recycling")
                .inputItems(input)
    ...
    private static void registerExtractorRecycling(UnificationEntry input, List<MaterialStack> materials, int multiplier, @Nullable TagPrefix prefix, Consumer<FinishedRecipe> provider) {
        ...
        if (prefix != null && prefix.secondaryMaterials().isEmpty()) {
            ...
            if (!m.hasProperty(PropertyKey.FLUID) || (prefix == dust && m.hasProperty(PropertyKey.BLAST))) {
                return;
            }
            EXTRACTOR_RECIPES.recipeBuilder(input, 0)
                    .inputItems(input)
        ...
        // Build the final Recipe.
        GTRecipeBuilder extractorBuilder = EXTRACTOR_RECIPES.recipeBuilder(input, 1)
                .inputItems(input)
        ...
        if (itemMs != null) {
            TagPrefix outputPrefix = itemMs.material().hasProperty(PropertyKey.INGOT) ? ingot : dust;
            var output = ChemicalHelper.get(outputPrefix, itemMs.material(), (int) (itemMs.amount() / M));
            if (!output.isEmpty()) {
                extractorBuilder.outputItems(output);
            }
        }
    ...
    private static void registerArcRecycling(UnificationEntry input, List<MaterialStack> materials, @Nullable TagPrefix prefix, Consumer<FinishedRecipe> provider) {
        // Block dusts from being arc'd instead of EBF'd
        MaterialStack ms = ChemicalHelper.getMaterial(input);
        if (prefix == dust && ms != null && ms.material().hasProperty(PropertyKey.BLAST)) {
            return;
        } else if (prefix == block) {
            if (ms != null && !ms.material().hasProperty(PropertyKey.GEM)) {
                ItemStack output = ChemicalHelper.get(ingot, ms.material().getProperty(PropertyKey.INGOT).getArcSmeltInto(), 9);
                ARC_FURNACE_RECIPES.recipeBuilder(input, 0)
                        .inputItems(input)
                        .outputItems(output)
        ...
        materials = combineStacks(materials.stream()
                .map(RecyclingRecipeHandler::getArcSmeltingResult)
        ...
        List<ItemStack> outputs = finalizeOutputs(
                materials,
                ARC_FURNACE_RECIPES.getMaxOutputs(ItemRecipeCapability.CAP),
                RecyclingRecipeHandler::getArcIngotOrDust
        );
        ...
        // Build the final Recipe.
        ARC_FURNACE_RECIPES.recipeBuilder(input, 1)
                .inputItems(input)
                .outputItems(outputs.toArray(ItemStack[]::new))
                .duration(calculateDuration(outputs))
                .EUt(GTValues.VA[GTValues.LV])
    ...
    private static MaterialStack getArcSmeltingResult(MaterialStack materialStack) {
        ...
        if (material.hasFlag(EXPLOSIVE)) {
            return new MaterialStack(Ash, amount / 16);
        }

        // If the Material is Flammable, return Ash
        if (material.hasFlag(FLAMMABLE)) {
            return new MaterialStack(Ash, amount / 8);
        }
    ...
    private static MaterialStack getGemArcSmeltResult(MaterialStack materialStack) {
        ...
        if (material.getMaterialComponents().stream()
                .anyMatch(stack -> stack.material() == Oxygen)) {
            return new MaterialStack(Ash, amount / 8);
        }

        // Else if the Gem Material has Carbon in it, return Carbon
        if (material.getMaterialComponents().stream()
                .anyMatch(stack -> stack.material() == Carbon)) {
            return new MaterialStack(Carbon, amount / 8);
        }

        // Else return Dark Ash
        return new MaterialStack(DarkAsh, amount / 8);
    }
    ...
    private static final List<TagPrefix> DUST_ORDER = ImmutableList.of(dust, dustSmall, dustTiny);
    private static final List<TagPrefix> INGOT_ORDER = ImmutableList.of(block, ingot, nugget);
    ...
    private static boolean isAshMaterial(MaterialStack ms) {
        return ms.material() == Ash || ms.material() == DarkAsh || ms.material() == Carbon;
    }
}
