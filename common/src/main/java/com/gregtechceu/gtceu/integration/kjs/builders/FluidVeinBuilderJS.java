    ...
    public FluidVeinBuilderJS biomes(int weight, String biomes) {
        this.biomes.add(
                new BiomeWeightModifier(biomes.startsWith("#") ?
                        BuiltinRegistries.BIOME.getOrCreateTag(TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(biomes.substring(1)))) :
                        HolderSet.direct(BuiltinRegistries.BIOME.getHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(biomes)))),
                        weight));
        return this;
    }
    ...
    public FluidVeinBuilderJS biomes(int weight, String... biomes) {
        ...
        for (String biome : biomes) {
            biomeKeys.add(biome.startsWith("#") ?
                    BuiltinRegistries.BIOME.getOrCreateTag(TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(biome.substring(1)))) :
                    HolderSet.direct(BuiltinRegistries.BIOME.getHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(biome))))
            );
        }
        HolderSet<Biome> holders = HolderSet.direct(biomeKeys.stream().flatMap(HolderSet::stream).toList());
        this.biomes.add(new BiomeWeightModifier(holders, weight));
        return this;
    }
