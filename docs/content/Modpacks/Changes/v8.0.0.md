...
- `GTElements.createAndRegister` takes a `ResourceLocation` id first, so addon elements register into your own namespace:
  ...
  ```
- The deprecated `IGTAddon.registerX()` callbacks are removed, and `GTCEuAPI.RegisterEvent` is deprecated
  - Your mod content classes should instead be loaded during your mod's startup.
...
## Switch to native MC registries

Previously, GT content was stored in our own custom implementation of registries. We have migrated to storing content within standard MC registries.

| Old (`GTRegistry`) | New (`MappedRegistry`) |
|---|---|
| `REG.register(id, value)` | `GTRegistries.register(REG, id, value)` |
| `REG.codec()` | `REG.byNameCodec()` |
| `REG.values()` / `.entries()` | `.stream()` / iterate the registry |
| `REG.getRegistryName()` | `REG.key().location()` |
| `REG.registerOrOverride(...)` | **no replacement** |

!!! warning
    Content should typically not be registered directly by calling `GTRegistries.register` or `Registry.register`. All content is typically registered through provided methods which do not require explicit registration.
