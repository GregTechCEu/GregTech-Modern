...
If you want to modify an existing machine's definition, you can do so using the `ModifyMachineEvent` forge event or the
`GTCEuStartupEvents.machineModification` KJS event. It is fired right before a machine gets registered, so that
its builder is still accessible through the `event.getBuilder()` method. For examples of methods that machine builders
have, see [Custom Machines](../Examples/Custom-Machines.md).
