...
"closest" inventory is not directly determined by the distance in blocks between source and destination. In stead, it is
...
equivalent non-restrictive pipes, allowing a pipe path to be forcibly made into the "longest" path and thus the final one
items can choose to go down.
...
crafted togther into a Quadruple or Nonuple Fluid Pipe. These pipes act as 4 or 9 separate piped all in a single block. 
